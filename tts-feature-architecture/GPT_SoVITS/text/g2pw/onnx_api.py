...
def download_and_decompress(model_dir: str = "G2PWModel/"):
    ...
    if not os.path.exists(model_dir):
        ...
        # Check if zip file already exists before downloading
        if not os.path.exists(zip_dir):
            ...
            try:
                ...
                raise FileNotFoundError(f"Failed to download G2PWModel: {e}. Zip file not found at {zip_dir}")
        else:
            print(f"Found existing zip file at {zip_dir}")

        if not os.path.exists(zip_dir):
            raise FileNotFoundError(f"G2PWModel zip file not found at {zip_dir}")

        print("Extracting g2pw model...")
        with zipfile.ZipFile(zip_dir, "r") as zip_ref:
            zip_ref.extractall(parent_directory)

        if os.path.exists(extract_dir) and not os.path.exists(extract_dir_new):
