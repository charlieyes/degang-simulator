...
try:
    ...
    # Fallback to jieba if jieba_fast is not available
    import jieba
...
if is_g2pw:
    ...
    # Make model_dir absolute path to avoid issues with working directory changes
    model_dir_path = os.path.join(parent_directory, "G2PWModel")
    # If relative path is used, resolve it relative to current file's directory
    if not os.path.isabs(model_dir_path):
        model_dir_path = os.path.abspath(model_dir_path)
    
    # Get model_source path - use TTS_BASE_PATH from env if available, otherwise use relative path
    bert_path_env = os.environ.get("bert_path")
    if bert_path_env:
        model_source = bert_path_env
    else:
        # Use TTS_BASE_PATH + relative path for portability
        tts_base_path = os.environ.get("TTS_BASE_PATH")
        if tts_base_path:
            # Construct absolute path: TTS_BASE_PATH + relative path
            model_source = os.path.join(tts_base_path, "tts-feature-architecture", "GPT_SoVITS", "pretrained_models", "chinese-roberta-wwm-ext-large")
            # Normalize the path
