...
# BigVGAN is only needed for v3/v4 models
try:
    from BigVGAN.bigvgan import BigVGAN
except ImportError:
    BigVGAN = None  # Not available, but we don't need it for v2/v2Pro models
# CNHubert import is optional - only needed if extracting features from new audio
try:
    from feature_extractor.cnhubert import CNHubert
except ImportError:
    CNHubert = None  # Not available, but we don't need it for pre-extracted features
...
# Audio super-resolution is optional
try:
    from tools.audio_sr import AP_BWE
except ImportError:
    AP_BWE = None  # Not available, but we don't need it for basic inference
...
class TTS_Config:
    ...
    def __init__(self, configs: Union[dict, str] = None):
        ...
        # Normalize version (case-insensitive, but preserve case for v2Pro/v2ProPlus)
        version_lower = version.lower()
        valid_versions = ["v1", "v2", "v3", "v4", "v2pro", "v2proplus"]
        if version_lower not in valid_versions:
            raise ValueError(f"Invalid version: {version}. Must be one of: v1, v2, v3, v4, v2Pro, v2ProPlus")
        # Map lowercase to proper case
        version_map = {"v1": "v1", "v2": "v2", "v3": "v3", "v4": "v4", "v2pro": "v2Pro", "v2proplus": "v2ProPlus"}
        version = version_map[version_lower]
        ...
        # Get cnhuhbert_base_path, but don't fallback to default if explicitly set to None
        self.cnhuhbert_base_path = self.configs.get("cnhuhbert_base_path")
        if "cnhuhbert_base_path" not in self.configs:
        ...
        # Only fallback to default if cnhuhbert_base_path was not explicitly set in config
        if "cnhuhbert_base_path" not in self.configs:
            if (self.cnhuhbert_base_path in [None, ""]) or (self.cnhuhbert_base_path and not os.path.exists(self.cnhuhbert_base_path)):
                self.cnhuhbert_base_path = self.default_configs[version]["cnhuhbert_base_path"]
        # If explicitly set to None, keep it as None
...
class TTS:
    ...
    def _init_models(
        ...
        # Skip CNHubert initialization - we have prompt_semantic pre-extracted
        if self.configs.cnhuhbert_base_path is not None:
            self.init_cnhuhbert_weights(self.configs.cnhuhbert_base_path)
        else:
            # Set CNHubert to None if not provided (for pre-extracted features)
            self.cnhuhbert_model = None
            print("CNHubert not initialized. Using pre-extracted features.")
        # self.enable_half_precision(self.configs.is_half)

    def init_cnhuhbert_weights(self, base_path: str):
        """Initialize CNHubert model (optional - only needed if extracting features from new audio)"""
        if base_path is None:
            self.cnhuhbert_model = None
            print("CNHuBERT base path is None, skipping CNHuBERT initialization.")
            return  # Skip if None
        if CNHubert is None:
            raise ImportError("CNHubert not available. Feature extraction from audio is not supported.")
        print(f"Loading CNHuBERT weights from {base_path}")
        self.cnhuhbert_model = CNHubert(base_path)
        self.cnhuhbert_model = self.cnhuhbert_model.eval()
        self.cnhuhbert_model = self.cnhuhbert_model.to(self.configs.device)
        # Only use half precision on CUDA, not on CPU
    ...
    def _get_ref_spec(self, ref_audio_path):
        # Use librosa instead of torchaudio to avoid torchcodec dependency
    ...
    def _set_prompt_semantic(self, ref_wav_path: str):
        if self.cnhuhbert_model is None:
            raise RuntimeError("CNHuBERT model is not initialized. Cannot extract prompt_semantic from audio. Please provide pre-extracted prompt_semantic or initialize CNHubert.")
        zero_wav = np.zeros(
        ...
        with torch.no_grad():
            ...
            # Match CNHubert model dtype - use float32 on CPU, half on CUDA
            if self.configs.is_half and str(self.configs.device) != "cpu":
                wav16k = wav16k.half()
            ...
            hubert_feature = self.cnhuhbert_model.model(wav16k.unsqueeze(0))["last_hidden_state"].transpose(
                1, 2
            ).float()  # Ensure float for VITS model (bias expects float32)
